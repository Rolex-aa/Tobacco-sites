...
export default function Navbar({ cart, updateQuantity }) {
  ...
  return (
    <header className="sticky top-0 z-[999] bg-nk-dark2/95 backdrop-blur-md border-b border-white/5 py-3">
      <div className="container mx-auto px-6 flex flex-wrap items-center justify-between gap-4">
        <div className="text-2xl font-extrabold text-nk-green-lt tracking-tight uppercase cursor-pointer" onClick={() => handleScroll('hero')}>
          🌿 NK Engineering <span className="text-nk-gold">& Works</span>
        </div>
        <nav className="flex items-center gap-7 flex-wrap relative">
          <button onClick={() => handleScroll('hero')} className="text-white/85 text-[17px] font-semibold hover:text-nk-gold transition-colors">Home</button>
          <Link to="/machine" className="text-white/85 text-[17px] font-semibold hover:text-nk-gold transition-colors">Machine</Link>
          <Link to="/parts" className="text-white/85 text-[17px] font-semibold hover:text-nk-gold transition-colors">Machine Parts</Link>
          <button onClick={() => handleScroll('repairing')} className="text-white/85 text-[17px] font-semibold hover:text-nk-gold transition-colors">Repairing</button>
          
          {/* Cart Icon - Show only on shop pages if items exist */}
          {isShopPage && cartCount > 0 && (
            <div className="relative">
              <button 
                onClick={() => setShowCart(!showCart)}
                className="relative p-2 text-white hover:text-nk-gold transition-colors flex items-center gap-1"
              >
